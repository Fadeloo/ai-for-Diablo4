...
assert(siteCoverage.frontendDataContracts?.some((contract) => contract.component === "GuideHeroLoadoutStrip" && contract.fields?.includes("gearSlots[].target.zhName") && contract.fields?.includes("gearSlots[].replaceable")), "Site coverage must describe the hero loadout strip contract");
...
assert(siteCoverage.frontendDataContracts?.some((contract) => contract.component === "PlannerCoreRequirementPanel" && contract.fields?.includes("coreRequirements[].powerName") && contract.fields?.includes("gearSlots[].alternatives[0]") && contract.fields?.includes("gameplay.loop[0]")), "Site coverage must describe the planner core requirement panel contract");
assert(siteCoverage.frontendDataContracts?.some((contract) => contract.component === "PlannerReplacementDeck" && contract.fields?.includes("gearSlots[].replaceable") && contract.fields?.includes("gearSlots[].alternatives[0].zhName")), "Site coverage must describe the planner replacement deck contract");
assert(siteCoverage.frontendDataContracts?.some((contract) => contract.component === "PlannerFullRouteDeck" && contract.fields?.includes("skillTree.pointOrder") && contract.fields?.includes("paragon.clickOrder") && contract.fields?.includes("paragon.clickOrder[].node")), "Site coverage must describe the planner full route deck contract");
assert(siteCoverage.frontendDataContracts?.some((contract) => contract.component === "PlannerGameplayDeck" && contract.fields?.includes("gameplay.opener") && contract.fields?.includes("gameplay.commonMistakes")), "Site coverage must describe the planner gameplay deck contract");
...
for (const forbidden of ["模型分", "先选目标", "rationale", "完整 BD 细节", "模型预估", "AI 思考", "模型推理", "候选配装", "配装规划", "配置速查", "完整技能", "完整巅峰", "完整打法", "看完整", "先按这几步", "路线待校准", "待回填", "待补来源", "待补赛季样本", "待补流派来源", "技能路线待补来源", "巅峰路线待补来源", "打法流程待补来源", "伤害样本待补来源", "核心件待补来源", "资料校验中", "资料待校准"]) {
...
assert(siteCoverage.frontendDataContracts?.some((contract) => contract.component === "SeasonClassArchetypeBoard" && contract.fields?.includes("formationDifficulty.label") && contract.fields?.includes("gearSlots[].replaceable") && contract.fields?.includes("ceiling.pit150Minutes")), "Site coverage must describe the class archetype board contract");
assert(frontendText.includes("renderSeasonClassArchetypeBoard") && frontendText.includes("season-class-board") && frontendText.includes("职业流派总览"), "BD library must expose a class-first archetype overview before individual cards");
assert(frontendText.includes("${renderSeasonClassArchetypeBoard(recommendedGuides)}\n    ${renderRecommendedBuildBoard(recommendedGuides)}\n    ${renderSeasonBuildMatrix(guides)}"), "BD default recommendation view must expose class archetype overview and the full class/archetype/mode matrix without requiring a view switch");
...
assert(frontendText.includes("renderGuideHeroLoadoutStrip") && frontendText.includes("guide-hero-loadout") && frontendText.includes("BD 首屏 11 部位配装"), "BD detail hero must expose all 11 gear positions before the detail layout");
assert(frontendText.includes("${renderGuideHeroExecutionStrip(guide)}\n        ${renderGuideHeroLoadoutStrip(guide)}\n      </header>"), "BD detail hero must place the 11-slot loadout immediately after the execution strip");
...
assert(frontendText.includes("renderBuildChapterIndex") && frontendText.includes("build-chapter-index") && frontendText.includes("BD 章节索引"), "BD detail must expose a fixed player-facing chapter index");
assert(frontendText.includes("${renderBuildChapterIndex(guide)}"), "BD planner sheet must show the chapter index near the top of the configuration page");
assert(frontendText.includes("renderPlannerCoreRequirementPanel") && frontendText.includes("planner-core-panel") && frontendText.includes("核心件与替换核对") && frontendText.includes("核心威能 / 暗金"), "BD planner sheet must expose core requirements, powers and replacements before long gear details");
assert(frontendText.includes("renderPlannerReplacementDeck") && frontendText.includes("planner-replacement-deck") && frontendText.includes("11 部位替换核对"), "BD planner sheet must expose every slot replacement state before the gear matrix");
assert(frontendText.includes("${renderBuildChapterIndex(guide)}\n      ${renderPlannerCoreRequirementPanel(guide)}\n      ${renderPlannerReplacementDeck(guide)}\n      ${renderPlannerGearMatrix(guide)}"), "BD planner sheet must show core requirements and replacement states between chapter index and gear matrix");
...
assert(frontendText.includes("renderPlannerGearMatrix") && frontendText.includes("配置页 11 部位配装矩阵") && frontendText.includes("11 部位配装矩阵"), "BD planner sheet must expose a front-loaded 11-slot gear matrix before icon-only overview");
assert(frontendText.includes("目标装备") && frontendText.includes("核心 / 替换") && frontendText.includes("威能或暗金") && frontendText.includes("词缀优先级"), "BD gear matrix must expose target item, core/replacement status, power and affix priority columns");
assert(frontendText.includes("${renderPlannerGearMatrix(guide)}\n      ${renderRouteOverview(guide)}"), "BD planner sheet must expose expanded skill and paragon route overview immediately after the gear matrix");
...
assert(frontendText.includes("renderPlannerFullRouteDeck") && frontendText.includes("planner-full-route-deck") && frontendText.includes("全量执行路线"), "BD planner sheet must expose full skill and paragon routes near the top of the configuration page");
assert(frontendText.includes("${renderRouteOverview(guide)}\n      ${renderPlannerFullRouteDeck(guide)}\n      ${renderPlannerGameplayDeck(guide)}"), "BD planner sheet must place the full route deck before gameplay, icon overview and long gear rows");
assert(frontendText.includes("renderPlannerGameplayDeck") && frontendText.includes("planner-gameplay-deck") && frontendText.includes("打法流程总览"), "BD planner sheet must expose complete gameplay flow near the top of the configuration page");
assert(frontendText.includes("${renderPlannerFullRouteDeck(guide)}\n      ${renderPlannerGameplayDeck(guide)}\n      <section class=\"planner-loadout-overview\""), "BD planner sheet must place the gameplay deck before icon overview and long gear rows");
