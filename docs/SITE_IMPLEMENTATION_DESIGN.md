...
| `ProgressionPlan` | 开荒到成型路线 | `guide.progression` | 展示 1-35、35-60、60+ 和用途专精阶段的装备、技能、巅峰、打法和替换规则 |
...
- `ProgressionPlan` 必须作为独立分区展示，不能只藏在职业开荒页；玩家进入某套 BD 后应知道从升级到终局如何过渡。
...
- `#equipment` 读取 `data/equipment/equipment-library.json` 的 `primarySlot`、`slotCandidates`、暗金特效、掉落来源和中文部位候选；部位来自暗黑核社区数据库交叉校验，必须用状态 `社区数据库参考` 展示，不能标成官方槽位。
...
- 装备库每条记录必须有中文部位候选、暗金特效和掉落来源，详情页必须显示部位、用途、适用场景和相关 BD。
- 装备详情必须显示暗金特效、完整词缀范围、掉落来源和槽位的字段级来源状态；完整范围词缀缺失时继续显示待回填。
