...
- `data/generated/build-simulations.json`：未来三个赛季窗口的冲层、速刷、日常配装预测矩阵，含 150 层速度估计、置信度、技能优先级、巅峰路线模板、装备策略和打法循环。
...
当前前端已经有配装模拟器、构筑手册、装备详情检索、职业开荒页、伤害实验室和 150 层预测页。配装模拟器可切换候选流派，并展示技能加点优先级、巅峰路线、装备栏位策略、推荐唯一装备、打法循环和开荒迁移。装备库卡片可展开查看固定词缀、配装用途、数据状态、补丁来源和图标来源。
