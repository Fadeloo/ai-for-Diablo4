...
const frontendDataContracts = [
  ...
  {
    component: "SeasonClassArchetypeBoard",
    zhName: "职业流派总览",
    source: "build-guides.builds + buildVersionModeOrder",
    fields: [
      "taxonomy.seasonId",
      "taxonomy.classId",
      "taxonomy.className",
    ...
    frontendUse: "BD 大厅默认视图先按职业列出每条流派轴，展示日常/速刷/冲层覆盖、推荐入口、成型难度、适用阶段、150 层参考、硬需求/可替换数量和核心件入口。"
  },
  {
  ...
  },
  {
    ...
    frontendUse: "BD 大厅按职业和流派横向比较日常、速刷、冲层版本；每个单元格展示成型难度、阶段、上限、核心件、技能起步、巅峰起步、打法入口，并直达装备、技能、巅峰和打法分区。"
  },
  {
  ...
  {
    component: "GuideHeroLoadoutStrip",
    zhName: "BD 首屏 11 部位配装条",
    ...
    fields: [
      ...
      "gearSlots[].target.zhName",
      "gearSlots[].target.itemId",
      "gearSlots[].required",
      "gearSlots[].replaceable"
    ],
    frontendUse: "BD 详情首屏在执行速览后直接展示 11 个装备位置、目标装备和硬需求/可替换状态，点击任意槽位进入装备分区。"
  },
  {
    component: "BuildChapterIndex",
    zhName: "BD 章节索引",
    source: "build-guides.builds[id] + guideSectionMeta",
    fields: [
      "guideCompleteness.label",
      "gearSlots",
      "gearSlots[].required",
      "gearSlots[].replaceable",
      "skillTree.pointOrder",
  ...
  {
    component: "PlannerGameplayDeck",
    zhName: "BD 配置页打法流程总览",
    source: "build-guides.builds[id].gameplay",
    fields: [
      "gameplay.opener",
      "gameplay.loop",
      "gameplay.boss",
      "gameplay.defense",
      "gameplay.speedFarm",
      "gameplay.commonMistakes"
    ],
    frontendUse: "BD 配置页在长装备明细前展示起手、主循环、首领、防御、速刷和常见错误，让玩家在同一页看到打法路线。"
  },
  {
  ...
  },
  {
    ...
    fields: [
      ...
      "gearSlots[].aspect.displayName",
      "gearSlots[].alternatives[0]",
      "skillTree.pointOrder[0..3]",
      "paragon.clickOrder[0..3]",
      "gameplay.opener[0]",
      "gameplay.loop[0]",
      "gameplay.boss[0]",
      "gameplay.defense[0]",
    ...
    frontendUse: "BD 总览提供成型前核对清单，按硬需求装备、可替换部位、技能起步、巅峰起步、打法窗口和来源状态组织。"
  },
  {
  ...
  },
  {
    ...
    fields: [
      ...
      "gearSlot.alternatives",
      "guide.ceiling",
      "source.verificationLevel"
    ],
    frontendUse: "装备详情先汇总关联 BD、常见职业、常见用途、常见部位和硬需求/可替换分布，再反查该装备在不同职业、流派和用途中的使用位置，并直达装备、技能和巅峰分区。"
  },
  {
