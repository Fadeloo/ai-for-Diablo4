...
- `data/generated/aspect-index.json`：71 条从结构化 BD 装备槽位汇总的威能索引，用于查询常见部位、核心度、替换状态和相关 BD。
- `data/generated/site-coverage.json`：玩家页数据覆盖摘要，记录 BD、装备、来源、存储层和字段缺口，来源页直接读取。
...
当前前端已经有 BD 大厅、独立 BD 详情页、装备详情检索、威能索引、职业开荒页、伤害实验室、150 层参考页和来源覆盖页。BD 大厅支持全部职业/全部用途的职业流派矩阵；BD 详情页按资料站结构展示全身装备速览、核心暗金或威能、可替换件、技能加点顺序、巅峰点击顺序、打法循环、变体和来源状态。装备库卡片可展开查看固定词缀、配装用途、数据状态、补丁来源和图标来源；威能索引可查看 BD 中出现的威能常见部位和相关 BD。
