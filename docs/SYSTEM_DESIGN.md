...
本文是继续实现前的总控设计。正式页面重构优先按 `docs/PLAYER_SITE_REDESIGN_PLAN.md` 执行；本文保留总控口径和核心数据契约，后续页面、组件、数据导入、AI 分析和验证都必须先有数据契约和组件边界，再写页面展示或导入脚本。
...
- 暗金特效、掉落来源和验证部位来自暗黑核社区数据库参考，字段级标注为社区来源；完整词缀范围、普通传奇威能和赛季改动仍需要继续接入来源后再展示为事实。
...
2. 装备库继续补完整词缀范围、传奇威能、游戏内截图校验和版本热修差异。
3. 巅峰从文本顺序升级为盘面和节点坐标。
